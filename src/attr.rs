...
impl Rewrite for ast::NestedMetaItem
{
  fn rewrite(&self, context: &RewriteContext<'_>, shape: Shape) -> Option<String>
  {
    match self
    {
      ...
      ast::NestedMetaItem::Lit(ref l) => rewrite_literal(context, l.as_token_lit(), l.span, shape),
    }
  }
}
      ...
      if let Some(ref meta) = self.meta()
      {
        ...
        if context.config.normalize_doc_attributes() && meta.has_name(sym::doc)
        {
          if let Some(ref literal) = meta.value_str()
          {
            ...
            let doc_comment_formatter = DocCommentFormatter::new(literal_str, comment_style);
...
impl Rewrite for [ast::Attribute]
{
  fn rewrite(&self, context: &RewriteContext<'_>, shape: Shape) -> Option<String>
  {
    ...
    loop
    {
      ...
      // Handle doc comments.
      let (doc_comment_len, doc_comment_str) = rewrite_initial_doc_comments(context, attrs, shape)?;
...
pub(crate) trait MetaVisitor<'ast>
{
  ...
  fn visit_meta_list(&mut self, _meta_item: &'ast ast::MetaItem, list: &'ast [ast::NestedMetaItem])
  {
    for nm in list
    {
