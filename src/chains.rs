...
impl ChainItemKind
{
  ...
  fn is_tup_field_access(expr: &ast::Expr) -> bool
  {
    match expr.kind
    {
      ast::ExprKind::Field(_, ref field) => field.name.to_string().chars().all(|c| c.is_digit(10)),
      _ => false,
    }
  }
        ...
        let types = if let Some(ref generic_args) = call.seg.args
        {
          if let ast::GenericArgs::AngleBracketed(ref data) = **generic_args
          {
            data
              ...
              {
                ast::AngleBracketedArg::Arg(ref generic_arg) => Some(generic_arg.clone()),
                _ => None,
              })
              .collect::<Vec<_>>()
          }
