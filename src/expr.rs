...
pub(crate) fn format_expr(
  ...
  let expr_rw = match expr.kind
    ...
    | ast::ExprKind::While(..) =>
      {
      to_control_flow(expr, expr_type).and_then(|control_flow| control_flow.rewrite(context, shape))
      }
    ...
    ast::ExprKind::Ret(Some(ref expr)) => rewrite_unary_prefix(context, "return ", &**expr, shape),
    ...
    ast::ExprKind::Index(ref expr, ref index) => rewrite_index(&**expr, &**index, context, shape),
...
fn rewrite_block(
  ...
  if let Some(ref result_str) = result
    ...
    if result_str.lines().count() <= 3
      {
      if let rw @ Some(_) = rewrite_single_line_block(context, &prefix, block, attrs, label, shape)
        {
        return rw;
        }
      }
...
impl<'a> ControlFlow<'a>
{
  ...
  fn rewrite_cond(
    ...
    let brace_overhead = if context.config.control_brace_style()
...
impl<'a> Rewrite for ControlFlow<'a>
{
  fn rewrite(&self, context: &RewriteContext<'_>, shape: Shape) -> Option<String>
    ...
    let alt_block_sep_string = format!("{}  ", &*alt_block_sep);
    let after_sep_AlwaysNextLine = alt_block_sep_string.as_str();

    let (cond_str, used_width) = self.rewrite_cond(context, shape, after_sep_AlwaysNextLine)?;
    ...
    if let Some(else_block) = self.else_block
      ...
      let between_kwd_else_block_comment = extract_comment(between_kwd_else_block, context, shape);
      ...
      let between_sep = match context.config.control_brace_style()
        {
        ControlBraceStyle::AlwaysNextLine => after_sep_AlwaysNextLine,
      ...
      let after_sep = match context.config.control_brace_style()
        {
        ControlBraceStyle::AlwaysNextLine if last_in_chain => after_sep_AlwaysNextLine,
        _ => " ",
        };
...
pub(crate) fn is_simple_expr(expr: &ast::Expr) -> bool
  ...
  match expr.kind
    ...
    ast::ExprKind::Repeat(ref lhs, ref rhs) => is_simple_expr(lhs) && is_simple_expr(&*rhs.value),
    _ => false,
    }
  }
...
pub(crate) fn can_be_overflowed_expr(
  ...
  match expr.kind
    ...
    ast::ExprKind::Array(..) | ast::ExprKind::Struct(..) =>
      {
      context.config.overflow_delimited_expr() || (context.use_block_indent() && args_len == 1)
      }
    ast::ExprKind::MacCall(ref mac) =>
    {
      match (
...
fn rewrite_struct_lit<'a>(
  ...
  let fields_str = if struct_lit_can_be_aligned(fields, has_base_or_rest)
    ...
    let rewrite = |item: &StructLitField<'_>| match *item
      ...
      StructLitField::Base(expr) =>
      {
        // 2 = ..
        expr
          .rewrite(context, v_shape.offset_left(2)?)
          .map(|s| format!("..{}", s))
        }
  ...
  let fields_str = wrap_struct_field(context, attrs, &fields_str, shape, v_shape, one_line_width)?;
...
pub(crate) fn rewrite_assign_rhs_expr<R: Rewrite>(
  ...
  {
  let last_line_width = last_line_width(lhs).saturating_sub(
    if lhs.contains('\n')
      {
      shape.indent.width()
      }
...
///    doesn't
pub(crate) fn prefer_next_line(orig_rhs: &str, next_line_rhs: &str, rhs_tactics: RhsTactics)
  -> bool
  {
