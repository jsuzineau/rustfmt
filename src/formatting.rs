...
{
  pub(crate) fn from_span(span: Span, parse_sess: &ParseSess, kind: ErrorKind) -> FormattingError
  {
    FormattingError {
...
impl<'a> FormatLines<'a>
{
  ...
  fn new_line(&mut self, kind: FullCodeCharKind)
  {
    if self.format_line
    {
      ...
      {
        if self.should_report_error(kind, &ErrorKind::TrailingWhitespace) && !self.is_skipped_line()
        {
          self.push_err(
            ErrorKind::TrailingWhitespace,
            kind.is_comment(),
  ...
  fn should_report_error(&self, char_kind: FullCodeCharKind, error_kind: &ErrorKind) -> bool
  {
    ...
    match error_kind
    {
      ErrorKind::LineOverflow(..) => self.config.error_on_line_overflow() && allow_error_report,
