...
fn flatten_use_trees(use_trees: Vec<UseTree>, import_granularity: ImportGranularity)
  -> Vec<UseTree>
{
...
impl UseTree
{
  ...
  ) -> Option<String>
  {
    ...
    match self.attrs
    {
      Some(ref attrs) if !attrs.is_empty() =>
      {
        ...
        combine_strs_with_missing_comments(context, &attr_str, &use_str, span, shape, allow_extend)
      }
      _ => Some(use_str),
    }
  }
  ...
  ) -> UseTree
  {
    ...
    let leading_modsep = context.config.edition() >= Edition::Edition2018 && a.prefix.is_global();
      ...
      UseTreeKind::Nested(ref list) =>
      {
        ...
        let kind = UseSegmentKind::List(
          list
            .iter()
            .zip(items)
            .map(|(t, list_item)| Self::from_ast(context, &t.0, Some(list_item), None, None, None))
            .collect(),
        );
        result.path.push(UseSegment { kind, version });
      }
...
impl Ord for UseSegment
{
  fn cmp(&self, other: &UseSegment) -> Ordering
  {
    ...
    match (&self.kind, &other.kind)
    {
      (Slf(ref a), Slf(ref b)) | (Super(ref a), Super(ref b)) | (Crate(ref a), Crate(ref b)) =>
      {
        match (a, b)
        {
          (Some(sa), Some(sb)) =>
...
mod test
{
  ...
  fn test_use_tree_flatten()
  {
    ...
    assert_eq!(
      parse_use_tree("a::b::{c::{d, e, f}, g, h::{i, j, k}}").flatten(ImportGranularity::Item),
      parse_use_trees![
        "a::b::c::d",
        "a::b::c::e",
        "a::b::c::f",
  ...
  fn test_use_tree_ord()
  {
    ...
    assert!(
      parse_use_tree("foo::{Bar, Baz}").normalize() < parse_use_tree("{Bar, Baz}").normalize()
    );
