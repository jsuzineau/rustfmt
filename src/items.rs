...
impl<'a> FmtVisitor<'a>
{
  ...
  pub(crate) fn single_line_fn(
    ...
    let res =
      Stmt::from_ast_node(block.stmts.first()?, true).rewrite(&self.get_context(), self.shape())?;
  ...
  pub(crate) fn visit_struct(&mut self, struct_parts: &StructParts<'_>)
    ...
    let rewrite =
      format_struct(&self.get_context(), struct_parts, self.block_indent, None).map(|s| {
        if is_tuple
          {
          s + ";"
          }
  ...
  pub(crate) fn visit_enum(
    ...
    {
    let enum_header = format_header(&self.get_context(), "enum ", ident, vis, self.block_indent);
  ...
  fn visit_impl_items(&mut self, items: &[ptr::P<ast::AssocItem>])
    ...
    if self.get_context().config.reorder_impl_items()
      ...
      let need_empty_line = |a: &ast::AssocItemKind, b: &ast::AssocItemKind| match (a, b)
          {
        (Type(lty), Type(rty)) if both_type(&lty.ty, &rty.ty) || both_opaque(&lty.ty, &rty.ty) =>
            {
          false
          }
        (Const(..), Const(..)) => false,
    ...
    let lhs = match comment_span
        ...
        if contains_comment(context.snippet_provider.span_to_snippet(comment_span)?) =>
      {
        let comment_shape = if has_where
          ...
          {
          Shape::indented(indent, context.config).block_left(context.config.tab_spaces())?
          };
...
impl<'a> StaticParts<'a>
{
  pub(crate) fn from_item(item: &'a ast::Item) -> Self
    ...
    let (defaultness, prefix, ty, mutability, expr) = match item.kind
        {
      ast::ItemKind::Static(ref ty, mutability, ref expr) => (None, "static", ty, mutability, expr),
  ...
  pub(crate) fn from_trait_item(ti: &'a ast::AssocItem) -> Self
    ...
    let (defaultness, ty, expr_opt) = match ti.kind
        {
      ast::AssocItemKind::Const(defaultness, ref ty, ref expr_opt) => (defaultness, ty, expr_opt),
      _ => unreachable!(),
      };
    StaticParts {
      prefix: "const",
...
fn rewrite_where_keyword(
  ...
  let allow_single_line =
    where_clause_option.allow_single_line && comment_before.is_empty() && comment_after.is_empty();
...
impl Rewrite for ast::ForeignItem
{
  fn rewrite(&self, context: &RewriteContext<'_>, shape: Shape) -> Option<String>
    ...
    let item_str = match self.kind
      ...
      ast::ForeignItemKind::Fn(ref fn_kind) =>
        ...
        if let Some(ref body) = body
          ...
          visitor.visit_fn(
            visit::FnKind::Fn(fn_ctxt, self.ident, sig, &self.vis, generics, Some(body)),
            &sig.decl,
            self.span,
            defaultness,
            Some(&inner_attrs),
    ...
    combine_strs_with_missing_comments(context, &attrs_str, &item_str, missing_span, shape, false)
    }
}
...
fn rewrite_attrs(
  ...
  let allow_extend = if attrs.len() == 1
    ...
    !attrs.first().unwrap().is_doc_comment() && context.config.inline_attribute_width() >= line_len
    }
    else
    {
    false
