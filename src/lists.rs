...
use crate::utils::{
  count_newlines, first_line_width, last_line_width, mk_sp, starts_with_newline, unicode_str_width,
};
  ...
  T: AsRef<ListItem>,
{
  ...
  while let Some((i, item)) = iter.next()
  {
    ...
    match tactic
    {
      ...
      DefinitiveListTactic::Vertical if !first && !inner_item.is_empty() && !result.is_empty() =>
      {
        result.push('\n');
        result.push_str(indent_str);
      }
    ...
    if tactic != DefinitiveListTactic::Horizontal && item.post_comment.is_some()
    {
      ...
      if !starts_with_newline(comment)
      {
        if formatting.align_comments
        {
          ...
          if first_line_width(&formatted_comment) + last_line_width(&result) + comment_alignment + 1
            > formatting.config.max_width()
          {
            item_max_width = None;
        ...
        if !formatting.align_comments
          || (last && item_max_width.is_some() && !separate && !formatting.separator.is_empty())
        {
          result.push(' ');
        }
      }
  ...
  F3: Fn(&T) -> Option<String>,
{
  ...
  fn next(&mut self) -> Option<Self::Item>
  {
    self.inner.next().map(|item| {
      ...
      let comment_end = get_comment_end(post_snippet, self.separator, self.terminator, is_last);
      let new_lines = has_extra_newline(post_snippet, comment_end);
      let post_comment = extract_post_comment(post_snippet, comment_end, self.separator, is_last);
