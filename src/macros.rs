...
) -> Option<String>
{
  ...
  if ts.is_empty() && !has_comment
  {
    return match style
    {
      ...
      Delimiter::Bracket if position == MacroPosition::Item => Some(format!("{}[];", macro_name)),
...
impl MacroArgKind
{
  ...
  ) -> Option<String>
  {
    ...
    match *self
    {
      ...
      MacroArgKind::Delimited(delim_tok, ref args) => rewrite_delimited_inner(delim_tok, args)
...
) -> Option<String>
{
  ...
  while let Some(arg) = iter.next()
  {
    ...
    {
      let space_before_dollar = !arg.kind.ends_with_space() && next_arg.kind.starts_with_dollar();
...
/// failed).
pub(crate) fn convert_try_mac(mac: &ast::MacCall, context: &RewriteContext<'_>)
  -> Option<ast::Expr>
{
...
/// }
/// ```
fn format_lazy_static(context: &RewriteContext<'_>, shape: Shape, ts: TokenStream)
  -> Option<String>
{
