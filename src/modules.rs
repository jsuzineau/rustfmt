...
impl<'ast, 'sess, 'c> ModResolver<'ast, 'sess>
{
  ...
  ) -> Result<(), ModuleResolutionError>
  {
    ...
    {
      (Some(Cow::Borrowed(ast::ModKind::Loaded(items, _, _))), _) => self.visit_mod_from_ast(items),
  ...
  ) -> Result<Option<SubModKind<'c, 'ast>>, ModuleResolutionError>
  {
      ...
      .default_submod_path(mod_name, relative, &self.directory.path)
    {
      ...
      }) =>
      {
        ...
        {
          Ok((ref attrs, _, _)) if contains_skip(attrs) => Ok(None),
          Ok((attrs, items, span)) if outside_mods_empty => Ok(Some(SubModKind::External(
            file_path,
            dir_ownership,
            Module::new(
              span,
      ...
      Err(e) => match e
      {
        ModError::FileNotFound(_, default_path, _secondary_path) => Err(ModuleResolutionError {
          module: mod_name.to_string(),
