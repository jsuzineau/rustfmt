...
impl Emitter for SilentOnIgnoredFilesEmitter
{
  ...
  fn emit_diagnostic(&mut self, db: &Diagnostic)
  {
    ...
    if let Some(primary_span) = &db.span.primary_span()
    {
      ...
      if let rustc_span::FileName::Real(rustc_span::RealFileName::LocalPath(ref path)) = file_name
      {
        if self
          .ignore_path_set
...
mod tests
{
  ...
  mod emitter
  {
    ...
    ) -> SilentOnIgnoredFilesEmitter
    {
      ...
      let ignore_path_set =
        Lrc::new(IgnorePathSet::from_ignore_list(&ignore_list.unwrap_or_default()).unwrap());
      SilentOnIgnoredFilesEmitter {
        has_non_ignorable_parser_errors: false,
        source_map,
    ...
    fn handles_fatal_parse_error_in_ignored_file()
    {
      ...
      let source_map = Lrc::new(SourceMap::new(FilePathMapping::empty()));
      let source = String::from(r#"extern "system" fn jni_symbol!( funcName ) ( ... ) -> {} "#);
