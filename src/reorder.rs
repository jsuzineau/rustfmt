...
impl ReorderableItemKind
{
  fn from(item: &ast::Item) -> Self
  {
    match item.kind
    {
      _ if contains_macro_use_attr(item) | contains_skip(&item.attrs) => ReorderableItemKind::Other,
  ...
  fn is_regroupable(self, config: &Config) -> bool
  {
    match self
    {
      ReorderableItemKind::ExternCrate | ReorderableItemKind::Mod | ReorderableItemKind::Other =>
      {
        false
      }
...
impl<'b, 'a: 'b> FmtVisitor<'a>
{
  ...
  ) -> usize
  {
    ...
    if at_least_one_in_file_lines && !items.is_empty()
    {
      ...
      let rw =
        rewrite_reorderable_or_regroupable_items(&self.get_context(), items, self.shape(), span);
