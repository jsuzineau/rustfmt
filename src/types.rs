  ...
  <I::Item as Deref>::Target: Rewrite + Spanned + 'a,
{
  ...
  let args =
    if tactic == DefinitiveListTactic::Horizontal || !context.use_block_indent() || is_inputs_empty
    {
      format!("({})", list_str)
    }
    else
  ...
  match gen_args
  {
    ast::GenericArgs::AngleBracketed(ref data) if !data.args.is_empty() =>
    {
      let args = data
        ...
        .map(|x| match x
        {
          ast::AngleBracketedArg::Arg(generic_arg) => SegmentParam::from_generic_arg(generic_arg),
          ast::AngleBracketedArg::Constraint(constraint) => SegmentParam::Binding(constraint),
        })
        .collect::<Vec<_>>();
...
impl Rewrite for ast::PolyTraitRef
{
  fn rewrite(&self, context: &RewriteContext<'_>, shape: Shape) -> Option<String>
  {
    if let Some(lifetime_str) = rewrite_lifetime_param(context, shape, &self.bound_generic_params)
    {
      // 6 is "for<> ".len()
...
fn is_generic_bounds_in_order(generic_bounds: &[ast::GenericBound]) -> bool
{
  ...
  {
    (Some(last_trait_index), Some(first_lifetime_index)) => last_trait_index < first_lifetime_index,
    _ => true,
  }
}
...
) -> Option<String>
{
      ...
      else
      {
        ...
        match leading_span
        {
          Some(ls) if has_leading_comment => (
            ...
            combine_strs_with_missing_comments(context, joiner, bound_str, ls, shape, true)?,
          ),
          _ => (
            is_bound_extendable(bound_str, item),
  ...
  let retry_with_force_newline = match context.config.version()
  {
    Version::One =>
    {
      !force_newline && items.len() > 1 && (result.0.contains('\n') || result.0.len() > shape.width)
    }
...
pub(crate) fn can_be_overflowed_type(context: &RewriteContext<'_>, ty: &ast::Ty, len: usize)
  -> bool
{
  match ty.kind
  {
