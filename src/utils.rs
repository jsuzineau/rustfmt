...
use rustc_ast::ast::{
  self, Attribute, MetaItem, MetaItemKind, NestedMetaItem, NodeId, Path, Visibility, VisibilityKind,
};
use rustc_ast::ptr;
...
pub(crate) fn is_same_visibility(a: &Visibility, b: &Visibility) -> bool
{
  match (&a.kind, &b.kind)
  {
    (VisibilityKind::Restricted { path: p, .. }, VisibilityKind::Restricted { path: q, .. }) =>
    {
...
pub(crate) fn semicolon_for_stmt(context: &RewriteContext<'_>, stmt: &ast::Stmt) -> bool
{
  match stmt.kind
  {
    ...
    {
      ast::ExprKind::While(..) | ast::ExprKind::Loop(..) | ast::ExprKind::ForLoop(..) => false,
  ...
  let min_prefix_space_width = lines
    .filter_map(|(kind, line)| {
      ...
      let new_veto_trim_value = (kind == FullCodeCharKind::InString
        || (config.version() == Version::Two && kind == FullCodeCharKind::InStringCommented))
        && !line.ends_with('\\');
