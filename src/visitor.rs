...
impl<'b, 'a: 'b> FmtVisitor<'a>
{
    ...
    for (kind, offset, sub_slice) in CommentCodeSlices::new(comment_snippet)
    {
      ...
      match kind
      {
        CodeCharKind::Comment =>
        {
          ...
          let mut comment_shape = Shape::indented(self.block_indent, config).comment(config);
          if self.config.version() == Version::Two && comment_on_same_line
          {
            ...
            match sub_slice.find('\n')
            {
              ...
              Some(offset) =>
              {
                ...
                let comment_str = rewrite_comment(other_lines, false, comment_shape, config);
                match comment_str
                {
          ...
          else
          {
            if comment_on_same_line
            {
              // 1 = a space before `//`
              let offset_len =
                1 + last_line_width(&self.buffer).saturating_sub(self.block_indent.width());
              match comment_shape
      ...
      match item.kind
      {
        ...
        ast::ItemKind::Fn(ref fn_kind) =>
        {
          ...
          else
          {
            let indent = self.block_indent;
            let rewrite =
              self.rewrite_required_fn(indent, item.ident, sig, &item.vis, generics, item.span);
  ...
  fn visit_assoc_item(&mut self, visitor_kind: &ItemVisitorKind<'_>)
  {
    ...
    match (&ai.kind, visitor_kind)
    {
      ...
      (ast::AssocItemKind::Fn(ref fn_kind), _) =>
      {
        ...
        {
          let indent = self.block_indent;
          let rewrite = self.rewrite_required_fn(indent, ai.ident, sig, &ai.vis, generics, ai.span);
  ...
  )
  {
    ...
    if let ast::ModKind::Loaded(ref items, ast::Inline::Yes, ref spans) = mod_kind
    {
      ...
      let body_snippet = self.snippet(mk_sp(mod_lo, source!(self, inner_span).hi() - BytePos(1)));
